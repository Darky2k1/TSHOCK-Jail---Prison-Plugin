        ...
        public void OnChat(messageBuffer msg, int ply, string text, HandledEventArgs e)
        {
            ...
            if (cmd == "/warp")
            {
                if (text.Split(' ').Length > 1)
                {
                    ...
                    {
                        if (Players[GetPlayerIndex(ply)].rulesMode && !tsplr.Group.HasPermission("jail") && !currentregionlist.Contains("jail") && JPConfig.jailmode)
                        {
                            tsplr.SendMessage("You Can't Teleport Without Reading / Following The Rules!", Color.Red);
                            e.Handled = true;
                            return;
                        }
                        if (currentregionlist.Contains("jail") && !tsplr.Group.HasPermission("jail") && Players[GetPlayerIndex(ply)].rulesMode && JPConfig.jailmode)
                        {
